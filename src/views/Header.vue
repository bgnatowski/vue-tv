<template>
  <header class="header">
    <h1 class="logo">VueTV</h1>
    <nav class="nav">
      <form class="search-form">
        <label for="search-input" class="visually-hidden">Szukaj</label>
        <input id="search-input" type="text" class="search-input" placeholder="Szukaj" />
      </form>
      <button class="nav-button" aria-label="Notifications">
        <img src="https://cdn.builder.io/api/v1/image/assets/TEMP/420e51d0e7889e609e38e486d0d5224d293a4112247531610d588c25b7a83f96?apiKey=7836e9d46c0a409dbac8e5fb7888aa0f&" alt="" class="nav-icon" />
      </button>
      <button class="nav-button" aria-label="User profile">
        <img src="https://cdn.builder.io/api/v1/image/assets/TEMP/69bd87dfedf11c3b126dec2696f718425195cbb9c3b42565a3e3664873a602d7?apiKey=7836e9d46c0a409dbac8e5fb7888aa0f&" alt="" class="nav-icon" />
      </button>
    </nav>
  </header>
</template>

<style scoped>
.header {
  background-color: #fff;
  box-shadow: 0 4px 13px rgba(0, 0, 0, 0.25);
  display: flex;
  gap: 20px;
  padding: 25px 35px;
}

@media (max-width: 991px) {
  .header {
    flex-wrap: wrap;
    padding: 0 20px;
  }
}

.logo {
  color: #3dccc7;
  flex-grow: 1;
  font: 400 48px Bruno Ace, sans-serif;
  margin: auto 0;
}

@media (max-width: 991px) {
  .logo {
    font-size: 40px;
  }
}

.nav {
  align-items: center;
  color: #848484;
  display: flex;
  font-size: 28px;
  font-weight: 600;
  gap: 20px;
  justify-content: space-between;
}

@media (max-width: 991px) {
  .nav {
    flex-wrap: wrap;
    max-width: 100%;
  }
}

.search-form {
  align-self: stretch;
  box-shadow: 0 4px 13px 0 rgba(0, 0, 0, 0.15);
  display: flex;
  flex-direction: column;
  justify-content: center;
}

@media (max-width: 991px) {
  .search-form {
    max-width: 100%;
  }
}

.search-input {
  align-items: start;
  background-color: #fff;
  border-radius: 36px;
  font-family: Red Hat Display, sans-serif;
  justify-content: center;
  padding: 22px 60px 22px 154px;
}

@media (max-width: 991px) {
  .search-input {
    max-width: 100%;
    padding: 0 20px 0 30px;
  }
}

.nav-button {
  align-self: stretch;
  background: none;
  border: none;
  cursor: pointer;
  margin: auto 0;
  padding: 0;
}

.nav-icon {
  aspect-ratio: 1;
  object-fit: contain;
  width: 44px;
}

.visually-hidden {
  border: 0;
  clip: rect(0 0 0 0);
  height: 1px;
  margin: -1px;
  overflow: hidden;
  padding: 0;
  position: absolute;
  white-space: nowrap;
  width: 1px;
}
</style>